...
def parse_args():
    ...
    parser.add_argument("-l", "--label", type=int, dest="label", default=None)
    parser.add_argument("--l3", dest="layer3", action="store_true")
...
if __name__ == "__main__":
    ...
    if args.layer3:
        if args.label is not None:
            print("Ignoring MPLS label %s" % args.label)
        print("%s <-- %s" % (str(addr_dst), args.msg))
        answ, peer = sendto(addr_dst, args.msg)
        print("%s --> %s" % (str(peer), answ.decode("utf-8")))
    else:
        pkt = udp(addr_src, addr_dst, args.msg, args.label)
        print("send\n%s" % dump_hex(pkt.data))
        answ = send(args.iface, pkt)
        print("recv\n%s" % dump_hex(bytearray(answ)))
