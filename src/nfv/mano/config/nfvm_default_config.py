...
def get_nfvm_default_config(datapath=None, data_port=1):
    if datapath is None:
    ...
    yield ofproto_parser.OFPFlowMod(
        ...
        instructions=[
            ofproto_parser.OFPInstructionActions(
                ...
                [
                    ofproto_parser.OFPActionOutput(port=ofproto.OFPP_CONTROLLER)
                ],
            ),
        ],
    )
    ...
    yield ofproto_parser.OFPFlowMod(
        ...
        instructions=[
            ofproto_parser.OFPInstructionActions(
                ofproto.OFPIT_APPLY_ACTIONS,
                [
                    ofproto_parser.OFPActionOutput(port=data_port)
                ],
            ),
        ],
    )
...
if __name__ == "__main__":
    with open("nfvm_default_config.json", "w") as fd:
        cfg = [msg.to_jsondict() for msg in get_nfvm_default_config()]
